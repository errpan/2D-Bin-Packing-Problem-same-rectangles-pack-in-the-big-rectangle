...
inline int localUpperBound(int iX, int iY) {
 ...
 * rectangle (L,W).
 */
void storeCutPoint(int L, int W, int x1, int x2, int y1, int y2) {
 ...
 */
int solve(int L, int W, int l, int w, int n, int numBlocks, int *L_, int *W_,
...
int BD(int L, int W, int l, int w, int n) {
  ...
  if(z_lb == 0 || z_lb == z_ub) {
    ...
    */
    ...
    for(index_x1 = 1; index_x1 < rasterX.size &&
          rasterX.points[index_x1] <= L / 2; index_x1++) {

      x1 = rasterX.points[index_x1];

      for(index_x2 = index_x1 + 1; index_x2 < rasterX.size &&
            rasterX.points[index_x2] + x1 <= L; index_x2++) {

        x2 = rasterX.points[index_x2];

        for(index_y1 = 1; index_y1 < rasterY.size &&
              rasterY.points[index_y1] < W; index_y1++) {

          y1 = rasterY.points[index_y1];

          for(index_y2 = index_y1 + 1; index_y2 < rasterY.size &&
                rasterY.points[index_y2] < W; index_y2++) {
    ...
    */
    ...
    for(index_x1 = 1; index_x1 < rasterX.size &&
      ...
      solved = solve(L, W, l, w, n, 2, L_, W_, &z_lb, z_ub, x1, x2, y1, y2);
      if(solved) {
        /* This problem was solved with optimality guarantee. */
        free(rasterX.points);
        free(rasterY.points);
    ...
    */
    ...
    for(index_y1 = 1; index_y1 < rasterY.size &&
          rasterY.points[index_y1] <= W / 2; index_y1++) {
...
void initialize(int L, int W, int l, int w) {
  ...
  while(i < rasterX.size && rasterX.points[i] <= L_n &&
        j < rasterY.size && rasterY.points[j] <= W_n) {
