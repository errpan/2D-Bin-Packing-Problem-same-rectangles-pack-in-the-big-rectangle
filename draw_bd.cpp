...
void drawHomogeneous(int x, int y, int dx, int dy) {
  int i, j;
...
void getSubproblems(CutPoint cutPoint, int L_[6], int W_[6], int L, int W) {
  int x1 = cutPoint.x1;
  int x2 = cutPoint.x2;
  int y1 = cutPoint.y1;
  int y2 = cutPoint.y2;
...
void drawRotation(int L, int W, int dx, int dy) {
  int L_[6], W_[6];
  int i, iX, iY;
  ...
  for(i = 1; i <= 5; i++) {
    ...
    if(L_[i] != 0 && W_[i] != 0 && 
      ...
      switch(i) {
      case 1: draw(L_[1], W_[1], dx + W_[4], dy + L_[2]); break;
      case 2: draw(L_[2], W_[2], dx + W_[5], dy        ); break;
      case 3: draw(L_[3], W_[3], dx + W_[4], dy + L_[5]); break;
      case 4: draw(L_[4], W_[4], dx        , dy + L_[5]); break;
      case 5: draw(L_[5], W_[5], dx        , dy        ); break;
      }
    }
    std::swap(L_[i], W_[i]);
  }
...
void drawNormal(int L, int W, int dx, int dy) {
  int i;
  int L_[6], W_[6];
  ...
  for(i = 1; i <= 5; i++) {
    if(L_[i] != 0 && W_[i] != 0 &&
       !((L_[i] == L && W_[i] == W) || (L_[i] == W && W_[i] == L))) {
      switch(i) {
      case 1: draw(L_[1], W_[1], dx        , dy + W_[4]); break;
      case 2: draw(L_[2], W_[2], dx + L_[1], dy + W_[5]); break;
      case 3: draw(L_[3], W_[3], dx + L_[1], dy + W_[4]); break;
      case 4: draw(L_[4], W_[4], dx        , dy        ); break;
      case 5: draw(L_[5], W_[5], dx + L_[4], dy        ); break;
      }
    }
  }
}
