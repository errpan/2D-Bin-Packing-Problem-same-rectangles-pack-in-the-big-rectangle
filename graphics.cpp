...
void readSolutionFile(int *n, int *L, int *W) {
  ...
  if(fscanf(solutionFile, "%d %d %d", n, L, W) != 3) {
    printf("ERROR: Error while reading solution file. Pattern does not match.\n");
    exit(0);
  }
  ...
  for(i = 0; i < *n; i++) {
    if(fscanf(solutionFile, "%d %d %d %d", &vertices[i][0], &vertices[i][1],
	      &vertices[i][2], &vertices[i][3]) != 4) {
      printf("ERROR: Error while reading solution file. Pattern does not match.\n");
      exit(0);
    }
  }
